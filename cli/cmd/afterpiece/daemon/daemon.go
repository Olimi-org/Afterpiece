...
import (
	...
	_ "modernc.org/sqlite" // for "sqlite" driver
...
func (d *Daemon) openDB() *sql.DB {
	...
	db, err := sql.Open("sqlite", fmt.Sprintf("file:%s?cache=shared&_journal=wal&_txlock=immediate", dbPath))
	if err != nil {
		fatal(err)
	}
...
func (d *Daemon) runDBMigrations(db *sql.DB) error {
	...
	instance, err := sqlitedb.WithInstance(db, &sqlitedb.Config{})
	if err != nil {
